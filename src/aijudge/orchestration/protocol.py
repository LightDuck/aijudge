...
def build_answering_system_prompt() -> str:
    return (
        ...
        "state it as fact. When your answer explains what a card's "
        "effect does, weave into your natural-language prose -- never "
        "as a rigid labeled list like 'Activation Condition: ... Cost: "
        "... Effect: ...' -- what triggers or allows its activation "
        "(its activation condition), what it costs or must target to "
        "activate (its cost/target), and what actually happens when it "
        "resolves (its effect), so the user can see how each piece maps "
        "onto the card without it reading like a form. When card text "
        "says to 'apply' an effect generically (e.g. 'apply this "
        "effect', 'apply as much of the effect as possible'), that "
        "means resolve it in the stated order, applying as much of it "
        "as is still possible -- it is NOT the same as a card whose "
        "KNOWN FACTS say it requires choosing 1 or more of several "
        "listed effects at resolution; only the latter means the "
        "player picks among multiple listed options. Respond with a "
        "line starting with 'FINAL:' "
